...
VERPSQL=$(psql --version 2>/dev/null | awk '{print $3}' | cut -d '.' -f 1)
...
# sudo usermod -d /var/lib/odoo odoo

# Verificar si el usuario 'odoo' existe
if getent passwd odoo > /dev/null; then
    echo "El usuario 'odoo' ya existe, continuando con la instalación..."
else
...
ODOO_CONF="/opt/odoo${VERSION}/conf/odoo.conf"
ODOO_STG_CONF="/opt/odoo${VERSION}_test/conf/stg.conf"

# Reemplazar valores existentes
sed -i 's/^limit_memory_hard *=.*/limit_memory_hard = 1342177280/' "$ODOO_CONF"
sed -i 's/^limit_memory_soft *=.*/limit_memory_soft = 1073741824/' "$ODOO_CONF"

sed -i 's/^limit_memory_hard *=.*/limit_memory_hard = 1342177280/' "$ODOO_STG_CONF"
sed -i 's/^limit_memory_soft *=.*/limit_memory_soft = 1073741824/' "$ODOO_STG_CONF"
